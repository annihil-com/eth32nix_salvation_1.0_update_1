...
void CEngine::DrawWarningMessages()
{
	...
	float size = 0.22f*scaleX;	// Text scale
	...
	VectorCopy(colorRed,fxphase.color[0]);
	VectorCopy(colorMdRed,fxphase.color[1]);
	VectorCopy(colorWhite,fxphase.color[2]);
	for (int i=0; i<3; i++)
	...
	if (eth32.cg.snap->ps.weaponstate == WEAPON_RELOADING) {			
	 ...
	 if (LowAmmoWeapon()) {
		if (clip < 3 && eth32.cg.snap->ps.weapon && eth32.cg.snap->ps.weaponstate != WEAPON_RELOADING) {
		fxphase.clips_phase++;
		if (fxphase.clips_phase == 2) 
			fxphase.clips_phase = 0;
		Draw.TextCenter(tx, ty*0.30, size, "LOW AMMO!", fxphase.color[fxphase.clips_phase], qtrue, qfalse, GUI_FONT, true);
		}
	 } else  
	    if (isAimableWeapon()) {
		if (clip < 13 && eth32.cg.snap->ps.weapon && eth32.cg.snap->ps.weaponstate != WEAPON_RELOADING) {
			fxphase.clips_phase++;
			if (fxphase.clips_phase == 2) 
				fxphase.clips_phase = 0;
			Draw.TextCenter(tx, ty*0.30, size, "LOW AMMO!", fxphase.color[fxphase.clips_phase], qtrue, qfalse, GUI_FONT, true);
		...
		Draw.TextCenter(tx, ty*0.25, size, "LOW HEALTH!", fxphase.color[fxphase.health_phase], qtrue, qfalse, GUI_FONT, true); 
	}
}
...
void CEngine::PlayerEsp(player_t *player)
{
	...
	if (Draw.worldToScreen(espOrigin, &player->screenX, &player->screenY)) {
		...
		if (Aimbot.grenadeTarget == player &&
			((eth32.settings.grenadeBot && eth32.cg.currentWeapon->attribs & WA_GRENADE) || (eth32.settings.rifleBot && eth32.cg.currentWeapon->attribs & WA_RIFLE_GRENADE))) {
			player->screenY -= 21;
		...
		switch(eth32.settings.classEspType) {
			...
			case CLS_STATIC: {
				...
				// Position
				y = player->screenY;
				x = (player->screenX - (iconSize * 3) / 2) + 7;
				y -= iconSize + 2;
 				Syscall.R_SetColor(iconColor);
 				Draw.Pic(x, y, iconSize, iconSize, eth32.cg.media.classIcons[player->Class]);
 				Draw.Pic(x + iconSize, y, iconSize, iconSize, eth32.cg.media.weaponIcons[player->currentState->weapon]);
 				Syscall.R_SetColor(NULL);
				break;
			}
			case CLS_DIST: {
