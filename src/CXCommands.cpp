...
void xcmd_RandMac()
{
	...
	strncpy(eth32.settings.sMac, buf, 16);
...
void xcmd_specstats()
{
	if (!eth32.settings.getSpeclist) {
		Syscall.UI_Print("^3speclisting not activated\n");
		return;
	}

	if (!eth32.cg.hooked){
		Syscall.UI_Print("^3Not connected\n");
		return;
	}
	...
	Syscall.UI_Print("^3Server: ^2%s ^dBytes rec: ^2%i ^dReq: ^2%i\n", bs, rq, server);
}
...
void xcmd_killspam()
{
	if (Syscall.UI_Argc() == 1){
		if (Tools.TotalKillspam() == 0) {
			Syscall.UI_Print("^3no kill spam loaded\n");
			return;
		}
...
void xcmd_multikillspam()
{
	if (Syscall.UI_Argc() == 1){
		if (Tools.TotalMultikillspam() == 0) {
			Syscall.UI_Print("^3no multikill spam loaded\n");
			return;
		}
...
void xcmd_suicidespam()
{
	if (Syscall.UI_Argc() == 1){
		if (Tools.TotalSuicidespam() == 0) {
			Syscall.UI_Print("^3no suicide spam loaded\n");
			return;
		}
...
void xcmd_deathspam()
{
	if (Syscall.UI_Argc() == 1){
		if (Tools.TotalDeathspam() == 0) {
			Syscall.UI_Print("^3no death spam loaded\n");
			return;
		}
